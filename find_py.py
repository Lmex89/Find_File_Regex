...
class GetFilesAndDirs:
    ...
    def get_dirs_and_files(self):
        ...
        read_write = os.access(
            self.string_path, os.W_OK) and os.access(self.string_path, os.R_OK)
        if not read_write:
            return {
                'dirs': [],
                'files': [],
    ...
    def get_file_matches(self):
        ...
        return {'parent_path':self.string_path,'dirs':self.list_of_dirs,'matches':self.list_matches}
...
regex = GetFilesAndDirs("/Users/sunwise/Documents", "cpp")
